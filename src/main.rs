...
fn main() {
    ...
    if args.len() != 6 {
        writeln!(std::io::stderr(),
                 "Usage: mandelbrot FILE THREADS PIXELS UPPERLEFT LOWERRIGHT")
            .unwrap();
        writeln!(std::io::stderr(),
                 "Example: {} mandel.png 2 1000x750 -1.20,0.35 -1,0.20",
                 args[0])
            .unwrap();
        std::process::exit(1);
    }
    let threads = usize::from_str(&args[2]).expect("error parsing thread count");
    let bounds = parse_pair(&args[3], 'x').expect("error parsing image dimensions");
    let upper_left = parse_pair(&args[4], ',').expect("error parsing upper left corner point");
    let lower_right = parse_pair(&args[5], ',').expect("error parsing lower right corner point");
    let mut pixels = vec![0; bounds.0 * bounds.1];
    multicore_render(&mut pixels, bounds, upper_left, lower_right, threads);
    write_bitmap(&args[1], &pixels, bounds).expect("error writing PNG file");
}


fn multicore_render(pixels: &mut [u8],
                    bounds: (usize, usize),
                    upper_left: (f64, f64),
                    lower_right: (f64, f64),
                    threads: usize) {
...
fn render(pixels: &mut [u8],
    ...
    for r in 0..bounds.1 {
        for c in 0..bounds.0 {
            ...
            let escapes_ = escapes(Complex {
                                       re: point.0,
                                       im: point.1,
                                   },
                                   255);
            pixels[r * bounds.0 + c] = match escapes_ {
                None => 0,
